...
public class peerClientThread1 extends Thread {
...
//                File folder = new File(cpath+"\\input_files");
        ...
        for (int i = 0; i < listOfFiles.length; i++) {
            if (listOfFiles[i].isFile()) {
                Client1.filenames.add((String) listOfFiles[i].getName());
                System.out.println("File " + listOfFiles[i].getName());
            } else if (listOfFiles[i].isDirectory()) {
                System.out.println("Directory " + listOfFiles[i].getName());
            }
        }
        request.setfilenames(Client1.filenames);
    ...
    private void RequestPeerList() {
        ...
        try {
            ...
            File file = new File("C:\\Users\\ducke\\Documents\\NetBeansProjects\\TestLTM\\src\\Client1\\input_files\\" + request.getFileName());
            if (file.exists()) {
...
//                System.out.println("\nPerformance speed " + df.format((megabytes * 1000000 * 1000) / output) + "MB/sec");
    ...
    private void sendObject2Server(cRequest request) {
        try {
            socket = new Socket(Client1.ServerIpAddress, Client1.ServerPort);
    ...
    private boolean sendObject2Client(cRequest request, int port) {
        try {
    ...
    private int receiveFile(cRequest request) throws UnknownHostException, IOException {
        ...
        System.out.println("Gửi yêu cầu đến peer");
        if(sendObject2Client(request, request.getPort())){
            System.out.println("Đã gửi yêu cầu");
            FileOutputStream OS = null;
            InputStream IS = null;
            // have to either get file or file not found
            IS = socket.getInputStream();
            OS = new FileOutputStream("C:\\Users\\ducke\\Documents\\NetBeansProjects\\TestLTM\\src\\Client1\\input_files\\" + request.getFileName());
            int bytesRead = 0;
