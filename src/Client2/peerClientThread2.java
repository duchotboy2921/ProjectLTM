...
public class peerClientThread2 extends Thread {
    ...
    private void Register() throws UnknownHostException {
        ...
        request.setPort(Client2.ClientPort);
        request.setHostName(Client2.ClientName);
        request.setRequestType(RequestType.Register);
        System.out.println("Send Object");

        File folder = new File("C:\\Users\\ducke\\Documents\\NetBeansProjects\\TestLTM\\src\\Client2\\input_files");
        File[] listOfFiles = folder.listFiles();
        for (int i = 0; i < listOfFiles.length; i++) {
            if (listOfFiles[i].isFile()) {
                Client2.filenames.add((String) listOfFiles[i].getName());
                System.out.println("File " + listOfFiles[i].getName());
            } else if (listOfFiles[i].isDirectory()) {
                System.out.println("Directory " + listOfFiles[i].getName());
            }
        }
        request.setfilenames(Client2.filenames);
    ...
    private void RequestPeerList() {
        ...
        try {
            ...
            if (serverResponse.getPeerList().size() == 0) {
                ...
                File file = new File("C:\\Users\\ducke\\Documents\\NetBeansProjects\\TestLTM\\src\\Client2\\input_files\\" + request.getFileName());
                if (file.exists()) {
...
//                System.out.println("\nPerformance speed " + df.format((megabytes * 1000000 * 1000) / output) + "MB/sec");
    ...
    private void sendObject2Server(cRequest request) {
        try {
            socket = new Socket(Client2.ServerIpAddress, Client2.ServerPort);
    ...
    private int receiveFile(cRequest request) throws UnknownHostException, IOException {
        ...
        OS = new FileOutputStream("C:\\Users\\ducke\\Documents\\NetBeansProjects\\TestLTM\\src\\Client2\\input_files\\" + request.getFileName());
        int bytesRead = 0;
